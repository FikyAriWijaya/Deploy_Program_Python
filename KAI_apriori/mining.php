...
function mining_process($db_object, $min_support, $min_confidence, $start_date, $end_date, $id_process){
    ...
    while($myrow = $db_object->db_fetch_array($result_trans)){
        ...
        $item_barang = $myrow['barang'].",";
        //mencegah ada jarak spasi
        $item_barang = str_replace(" ,", ",", $item_barang);
        $item_barang = str_replace("  ,", ",", $item_barang);
        $item_barang = str_replace("   ,", ",", $item_barang);
        $item_barang = str_replace("    ,", ",", $item_barang);
        $item_barang = str_replace(", ", ",", $item_barang);
        ...
        $dataTransaksi[$x]['barang'] = $item_barang;
        $barang = explode(",", $myrow['barang']);
        //all items
        foreach ($barang as $key => $value_barang) {
            //if(!in_array($value_barang, $item_list)){
            if(!in_array(strtoupper($value_barang), array_map('strtoupper', $item_list))){
                if(!empty($value_barang)){
                    $item_list[] = $value_barang;
                }
            }
        }
        $x++;
...
function get_count_itemset2($db_object, $atribut1, $atribut2, $start_date, $end_date) {
    $sql = "SELECT COUNT(transaction_date) AS jml, transaction_date 
            FROM transaksi 
            WHERE (barang='$atribut1' OR barang = '$atribut2') 
...
function get_count_itemset3($db_object, $atribut1, $atribut2, $atribut3, $start_date, $end_date) {
    $sql = "SELECT COUNT(transaction_date) AS jml, transaction_date FROM transaksi 
            WHERE (barang='$atribut1' OR barang = '$atribut2'  OR barang = '$atribut3') 
                AND transaction_date BETWEEN '$start_date' AND '$end_date' 
            GROUP BY transaction_date
...
function jumlah_itemset1($transaksi_list, $barang){
    $count = 0;
    foreach ($transaksi_list as $key => $data) {
        $items = ",".strtoupper($data['barang']);
        $item_cocok = ",".strtoupper($barang).",";
